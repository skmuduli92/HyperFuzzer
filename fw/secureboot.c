...
/*---------------------------------------------------------------------------*/
/* DEBUGGIN INFO FOR TRACE GENERATION */
...
enum status { UNDET, PASS, FAIL };

// public key hash
CODE unsigned char pkhash[20] = {0x37, 0x34, 0xA6, 0x83, 0x5F, 0xFC, 0xE0, 0x2B, 0xC6, 0xEE,
                                 0xCB, 0x81, 0x6C, 0x92, 0x6C, 0x7C, 0xBA, 0x79, 0xCB, 0x8F};
...
void main() {
  ...
  record_dbg(VERIF_HEADER_SIGN, 0);
  record_dbg(VERIF_BLOCK_HASH, 0);
  ...
  for (i = 0; i < num; i++) {
    ...
    if (moddata + size > boot + MAX_IM_SIZE || moddata + size < moddata)  // overflow
    {
      // addr_val = 0;
      pass = FAIL;
    ...
    if (size + ldaddr > MAX_PRG_SIZE || ldaddr + size < ldaddr) {
      // addr_val = 0;
      pass = FAIL;
  ...
  record_dbg(VERIF_HEADER_SIGN, pass);
  record_dbg(VERIF_BLOCK_HASH, pass);
